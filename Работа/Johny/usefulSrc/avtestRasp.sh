...
udpsrc caps="application/x-rtp, media=(string)audio, clock-rate=(int)8000, width=16, height=16, channels=1, channel-positions=1, payload=96" port=7000 ! rtpbin.recv_rtp_sink_1 \
rtpbin. ! queue ! rtppcmudepay ! mulawdec ! audioconvert ! autoaudiosink sync=false 
udpsrc port=7001 ! rtpbin.recv_rtp_sink_1
rtpbin.send_rtcp_src_1 ! udpsink host=$DEST port=7005 
